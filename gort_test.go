...
// https://github.com/vishr/web-framework-benchmark/blob/master/router_test.go
...
func loadGortRoutes(g *Router, routes []*TestRoute) {
	for _, r := range routes {
		switch r.Method {
		case "GET":
			g.GET(r.Path, gortHandler(r.Method, r.Path))
		case "POST":
			g.POST(r.Path, gortHandler(r.Method, r.Path))
		case "PUT":
			g.PUT(r.Path, gortHandler(r.Method, r.Path))
		case "DELETE":
			g.DELETE(r.Path, gortHandler(r.Method, r.Path))
		case "PATCH":
			g.PATCH(r.Path, gortHandler(r.Method, r.Path))
		}
	}
}
...
func BenchmarkGortGplusAPI(b *testing.B) {
	g := New()
	loadGortRoutes(g, gplusAPI)
	benchmarkRoutes(b, g, gplusAPI)
}

func BenchmarkGortParseAPI(b *testing.B) {
	g := New()
	loadGortRoutes(g, parseAPI)
